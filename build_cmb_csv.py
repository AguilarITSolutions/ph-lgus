#!/usr/bin/env python3
"""Flatten cities/municipalities/barangays geojson into cmb.csv (cities,
municipalities, barangays only -- regions/provinces are static, see
region_province.json) for `COPY ph_lgus(...) FROM 'cmb.csv' CSV HEADER`.
...
# provinces.geojson isn't exported here, but its ids are still needed to tell
# a real province apart from an NCR "district" ADM2 label.
province_ids = {feat["properties"]["psgc_id"] for feat in load("provinces.geojson")}
...
with open("cmb.csv", "w", newline="") as out:
    w = csv.writer(out)
    w.writerow(["psgc_id", "psgc_code", "psgc_name", "psgc_type", "parent_psgc_id"])
    for row in rows.values():
        w.writerow(row)

print(f"wrote {len(rows)} rows to cmb.csv")
